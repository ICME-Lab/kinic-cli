...
pub struct App {
    ...
    // In-TUI chat panel (to the right of inspector)
    pub chat_open: bool,
    ...
    pub chat_rx: mpsc::Receiver<String>,
}
...
impl App {
    ...
    pub fn analyze_project(&mut self, path: &Path) -> Result<()> {
        ...
        let is_workspace_root = manifest_path.exists()
            && std::fs::read_to_string(&manifest_path)
                .map(|s| s.contains("[workspace]"))
                .unwrap_or(false);
        if path.is_file() && path.extension().is_some_and(|ext| ext == "rs") {
            self.items = analyzer.analyze_file(path)?;
        } else if is_workspace_root && path.is_dir() {
            // Workspace roots often keep real code under member dirs (e.g. crates/, examples/).
    ...
    pub fn maybe_start_crate_doc_fetch(&mut self) {
        ...
        thread::spawn(move || {
            let result = crate::domain_rust::crates_io::fetch_crate_docs(&name);
    ...
    pub fn next_focus(&mut self) {
        self.focus = self.focus.next(self.chat_open);
    }

    pub fn prev_focus(&mut self) {
        self.focus = self.focus.prev(self.chat_open);
    }
    ...
    /// Build context string for the currently selected item (for Chat).
    pub fn build_chat_context(&self) -> Option<String> {
    ...
    /// Submit the current chat input (spawns thread, sets loading).
    pub fn submit_chat_message(&mut self) {
        ...
        thread::spawn(move || {
            ...
            let response = match output {
                ...
                Ok(o) => format!(
                    "Assistant error (exit {}): {}",
                    o.status,
                    String::from_utf8_lossy(&o.stderr)
                ),
                Err(e) => format!("Failed to run assistant command: {}", e),
            };
    ...
    /// Toggle chat panel; when opening with an item selected, focus chat.
    ...
    /// Rebuild UI DTO caches from current app state.
    pub fn refresh_ui_cache(&mut self) {
        self.ui_in_crate_items_view =
            self.current_tab == Tab::Crates && self.selected_installed_crate.is_some();
        self.ui_total_count = if self.ui_in_crate_items_view {
            self.installed_crate_items.len()
        } else {
            self.items.len()
        };

        self.ui_summaries = self
            .get_filtered_items()
            .into_iter()
            .map(item_to_summary)
            .collect();
        ...
        self.ui_dependency_node = if self.current_tab == Tab::Crates {
            if self.ui_in_crate_items_view {
                self.selected_installed_crate
                    .as_ref()
                    .map(installed_crate_to_node)
            } else if let Some(name) = self.selected_dependency_name() {
                if self.dependency_root_name() == Some(name.as_str()) {
                    self.crate_info.as_ref().map(crate_info_to_node)
                } else {
                    self.crate_docs_cache.get(&name).map(crate_doc_to_node)
                }
            } else {
                None
            }
...
fn oracle_ui_config() -> UiConfig {
    UiConfig {
        tabs: vec![
            TabSpec {
                id: TabId::new("types"),
                title: "Types".to_string(),
                search_placeholder: "Search types... (struct, enum, type)".to_string(),
            },
            TabSpec {
                id: TabId::new("functions"),
                title: "Functions".to_string(),
                search_placeholder: "Search functions...".to_string(),
            },
            TabSpec {
                id: TabId::new("modules"),
                title: "Modules".to_string(),
                search_placeholder: "Search modules...".to_string(),
            },
            TabSpec {
                id: TabId::new("crates"),
                title: "Crates".to_string(),
                search_placeholder: "Search crates... (filter by name)".to_string(),
            },
        ],
        ..UiConfig::default()
    }
