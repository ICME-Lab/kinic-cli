...
impl KinicProvider {
    ...
    fn current_records(&self) -> Vec<&KinicRecord> {
        ...
        if self.memories_mode == MemoriesMode::Results
            || self.query.is_empty()
            || (self.is_live() && self.memories_mode == MemoriesMode::Browser)
        {
    ...
    fn visible_memory_records(&self) -> Vec<&KinicRecord> {
        ...
        self.memory_records.iter().collect()
    }
    ...
    fn run_live_search(&mut self, scope: SearchScope) -> Option<CoreEffect> {
        ...
        thread::spawn(move || {
            ...
            let result = runtime.block_on(async move {
                ...
                let agent_result = bridge::build_search_agent(use_mainnet, auth.clone())
                    .await
                    .map_err(|error| error.to_string());
    ...
    fn handle_action(
        ...
        match action {
            ...
            CoreAction::SearchSubmit => {
                if self.is_live()
                    && let Some(effect) = self.run_live_search(state.search_scope)
                {
                    effects.push(effect);
                }
            }
