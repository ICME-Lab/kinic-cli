...
mod tests {
    ...
    fn set_tab_create_handles_mock_and_live_modes() {
        ...
        assert_eq!(mock_provider.create_cost_state, CreateCostState::Unavailable);
        ...
        assert!(effects.iter().any(|effect| matches!(effect, CoreEffect::Notify(_))));
    }
    ...
    fn poll_background_applies_refreshed_session_settings() {
        ...
        assert!(output.effects.iter().any(|effect| matches!(effect, CoreEffect::Notify(_))));
    }
    ...
    fn poll_background_discards_stale_search_results_after_context_changes() {
        ...
        for scenario in scenarios {
            ...
            assert_eq!(provider.memories_mode, MemoriesMode::Browser, "scenario: {scenario}");
            assert!(provider.result_records.is_empty(), "scenario: {scenario}");
            assert_eq!(provider.pending_search_context, None, "scenario: {scenario}");
    ...
    fn poll_background_cleans_pending_search_state_for_failure_and_disconnect() {
        ...
        disconnected_provider.pending_search_context = Some(pending_search_context(3, "aaaaa-aa", "alpha"));
