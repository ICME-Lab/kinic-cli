...
fn build_snapshot_shows_public_action_for_anonymous_reader() {
    ...
    assert_eq!(
        actions.body_lines,
        vec!["> Open public memory in browser".to_string()]
    );
}
...
#[test]
fn build_snapshot_shows_public_action_for_anonymous_writer() {
    let mut provider = KinicProvider::new(live_config());
    provider.memory_summaries = vec![MemorySummary {
        users: Some(vec![bridge::MemoryUser {
            principal_id: "anonymous".to_string(),
...
#[test]
fn build_snapshot_hides_public_action_without_anonymous_read_access() {
    for users in [
        None,
...
#[test]
fn build_snapshot_shows_public_and_remove_actions_for_public_manual_memory() {
    let mut provider = KinicProvider::new(live_config());
    provider.user_preferences.manual_memory_ids = vec!["aaaaa-aa".to_string()];
    provider.memory_summaries = vec![MemorySummary {
        users: Some(vec![bridge::MemoryUser {
    ...
    assert_eq!(
        actions.body_lines,
        vec![
            "  Open public memory in browser".to_string(),
...
#[test]
fn memory_content_open_selected_opens_public_and_remove_actions_for_public_manual_memory() {
    let mut provider = KinicProvider::new(live_config());
    provider.user_preferences.manual_memory_ids = vec!["aaaaa-aa".to_string()];
    provider.memory_summaries = vec![MemorySummary {
        users: Some(vec![bridge::MemoryUser {
    ...
    assert!(public_output.effects.iter().any(|effect| matches!(
        effect,
        CoreEffect::OpenExternal(url) if url == "https://memory.kinic.xyz/m/aaaaa-aa"
    )));

    let remove_output = provider
        .handle_action(
            &CoreAction::MemoryContentOpenSelected,
            &CoreState {
                current_tab_id: KINIC_MEMORIES_TAB_ID.to_string(),
