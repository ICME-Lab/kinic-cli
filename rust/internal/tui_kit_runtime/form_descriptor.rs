...
use crate::{
    CoreAction, CoreState, CreateModalFocus, InsertFormFocus, InsertMode, PickerContext,
...
const INSERT_FIELDS: [InsertFieldSpec; 7] = [
    ...
    InsertFieldSpec {
        field: FormFieldSpec {
            ...
            enter_command: FormCommand::OpenPicker(PickerContext::InsertTarget),
            accepts_input: false,
            supports_horizontal_change: false,
        },
        availability: InsertFieldAvailability::Always,
    ...
    InsertFieldSpec {
        field: FormFieldSpec {
            ...
            enter_command: FormCommand::OpenPicker(PickerContext::InsertTag),
            accepts_input: false,
            supports_horizontal_change: false,
        },
        availability: InsertFieldAvailability::Always,
...
pub fn form_command_to_action(kind: FormKind, command: FormCommand) -> Option<CoreAction> {
    match (kind, command) {
        ...
        (FormKind::Insert, FormCommand::OpenPicker(context)) => {
            Some(CoreAction::OpenPicker(context))
        }
...
pub fn core_action_to_form_command(action: &CoreAction) -> Option<(FormKind, FormCommand)> {
    match action {
        ...
        CoreAction::OpenPicker(context) => {
            Some((FormKind::Insert, FormCommand::OpenPicker(*context)))
        }
...
fn form_field_spec(focus: FormFocus, insert_mode: Option<InsertMode>) -> Option<FormFieldSpec> {
    match focus {
        FormFocus::Create(_) => CREATE_FIELDS
            .iter()
            .find(|field| field.focus == focus)
            .copied(),
        FormFocus::Insert(_) => {
