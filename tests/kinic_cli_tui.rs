...
fn prefs_help_mentions_json_contract_and_examples() {
    ...
    assert!(stdout.contains("show -> {\"default_memory_id\": string|null, \"saved_tags\": string[], \"manual_memory_ids\": string[], \"chat_overall_top_k\": integer, \"chat_per_memory_cap\": integer, \"chat_mmr_lambda\": integer, \"embedding_model_id\": string}"));
    assert!(stdout.contains("kinic-cli prefs set-default-memory --memory-id"));
    assert!(stdout.contains("kinic-cli prefs set-embedding-backend --model-id BAAI/bge-m3"));
}
...
fn prefs_show_help_mentions_return_shape() {
    ...
    assert!(stdout.contains("{\"default_memory_id\": string|null, \"saved_tags\": string[], \"manual_memory_ids\": string[], \"chat_overall_top_k\": integer, \"chat_per_memory_cap\": integer, \"chat_mmr_lambda\": integer, \"embedding_model_id\": string}"));
}
